...
const AdminDashboard = () =>{
    ...
    const fetchCounts = async () => {
        try {
            ...
            const teacherRes = await axios.get('/api/v1/teachers/getall');
            const classRes = await axios.get('/api/v1/class/getall');
            setCounts({
                ...
                <TopContent>
                    ...
                    <Section>
                        ...
                        <ul style={{ listStyle: 'none', padding: 0 }}>
                            {events.slice(0, 5).map(e => <li key={e._id || e.id} style={{ padding: '10px', borderBottom: '1px solid #eee' }}>{e.events}</li>)}
                        </ul>
                    </Section>
                </TopContent>
                    ...
                    <Section style={{ flex: 1 }}>
                        ...
                        <ul style={{ listStyle: 'none', padding: 0 }}>
                            {announcements.slice(0, 5).map(a => (
                                <li key={a._id || a.id} style={{ padding: '10px' }}>
