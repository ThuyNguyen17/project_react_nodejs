...
const CheckAttendanceSection = () => {
    ...
    const fetchAssignments = async () => {
        try {
            ...
            if (storedUser) {
                ...
                const response = await axios.get(`/api/v1/teaching-assignments/teacher/${user.teacherId || user.userId}`);
    ...
    const startSession = async () => {
        ...
        try {
            const response = await axios.post('/api/attendance/session/start', {
    ...
    const generateToken = async (sessionId) => {
        ...
        try {
            const response = await axios.post(`/api/attendance/session/${sessionId}/token`, { token });
            setCurrentSession(response.data);
        } catch (error) {
    ...
    const startMonitoring = (sessionId) => {
        const t = setInterval(async () => {
            try {
                const response = await axios.get(`/api/attendance/session/${sessionId}/attendances`);
    ...
    const closeSession = async () => {
        ...
        try {
            await axios.post(`/api/attendance/session/${currentSession.id}/close`);
            clearInterval(timer);
            setTimer(null);
