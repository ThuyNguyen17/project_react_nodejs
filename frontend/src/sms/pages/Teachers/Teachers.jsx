...
const TeacherSection = () =>{
    ...
    const handleAddTeacher = async (e) => {
        ...
        if(newTeacher.fullName.trim() !== '' && newTeacher.email.trim() !== ''){
            try{
                await axios.post('/api/v1/teachers', newTeacher);
                fetchTeachers(); 
    ...
    const handleUpdateTeacher = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`/api/v1/teachers/${editingTeacher._id || editingTeacher.id}`, editingTeacher);
                        ...
                        {Array.isArray(teachers) && teachers.map((teacher) => (
                            <TeacherItem key={teacher._id || teacher.id}>
                                ...
                                <div style={{ marginTop: '10px' }}>
                                    ...
                                    <button onClick={() => handleDeleteTeacher(teacher._id || teacher.id)} style={{ padding: '5px 10px', backgroundColor: '#dc3545', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Delete</button>
                                </div>
                            </TeacherItem>
                        ))}
                    </TeacherList>
