...
const AttendanceModal = ({ isOpen, onClose, assignmentId, date, period, semester, className }) => {
    ...
    // 2-second interval for fetching attendees and missing students
    useEffect(() => {
        if (!sessionId) return;

        const fetchData = async () => {
            try {
                const [attendancesData, missingData] = await Promise.all([
                    getAttendances(sessionId),
                    getMissingStudents(sessionId)
                ]);
    ...
    const formatTime = (timeStr) => {
        ...
        if (typeof timeStr === 'string') {
            ...
            // If it's already formatted time string, ensure it includes seconds
            if (timeStr.includes(':')) {
    ...
    const unattendedStudents = missingStudents; // Use API data instead of local filtering
