...
import { getAllClasses, getStudentsInClass, importStudentsToClass, exportStudentsFromClass } from "../../api/classApiV2";
import "./Students.css";
...
const AdminStudents = () => {
  ...
  const fetchClasses = async () => {
    ...
    try {
      ...
      const formattedClasses = data.map(c => {
        const className = c.displayName || c.className;
        return {
          ...c,
          name: className,
          className: className,
  ...
  const handleClassClick = (classItem) => {
    ...
    fetchStudentsByClass(classItem.name || classItem.displayName || classItem.className || classItem.id);
  };
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const blob = await exportStudentsFromClass(selectedClass.name || selectedClass.className || selectedClass.id);
  ...
  const handleImportExcel = async (e) => {
    ...
    try {
      await importStudentsToClass(selectedClass.name || selectedClass.id, file);
      ...
      await fetchStudentsByClass(selectedClass.name || selectedClass.id);
    } catch (err) {
            ...
            <tbody>
              {filteredStudents.map((student, index) => (
                <tr key={student.id}>
                  ...
                  <td>{student.dob}</td>
                  <td>{student.contact?.address || 'Chưa có'}</td>
                </tr>
              ))}
            </tbody>
          </table>
