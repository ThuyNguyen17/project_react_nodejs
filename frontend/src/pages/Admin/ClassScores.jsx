...
const ClassScores = () => {
  ...
  const fetchAssignments = async () => {
    try {
      const resp = await axios.get(`${API_BASE_URL}/teaching-assignments/all`);
  ...
  const fetchStudents = async () => {
    try {
      const resp = await axios.get(`${API_BASE_URL}/students/class/${encodeURIComponent(selectedClass)}`);
  ...
  const fetchScores = async () => {
    ...
    try {
      ...
      const resp = await axios.get(`${API_BASE_URL}/scores/teaching-assignment/${selectedAssignment}`);
      const scoreData = {};
        ...
        <Controls>
          <Select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)}>
            ...
            {classes.map(c => {
              ...
              let label = c.displayName || c.grade || (gLevel && cName ? (cName.startsWith(gLevel) ? cName : gLevel + cName) : cName);
