...
const AssignmentSubmissions = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // normalize _id to id for each submission
      const normalized = (submissionsData || []).map(s => ({
        ...s,
        id: s.id || s._id
      }));
      setSubmissions(normalized);

      // Fetch violations separately so it doesn't break the page if it fails
      try {
        const violationsData = await getViolationsByAssignment(assignmentId);
        setViolations(violationsData || []);
      } catch (vErr) {
        console.warn('Could not load violations:', vErr);
        setViolations([]);
      }
    } catch (err) {
