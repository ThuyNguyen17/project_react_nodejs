...
const StudentScanner = () => {
    ...
    const reverseGeocodeViaBackend = async (lat, lon) => {
        const res = await fetch(`${BASE_URL}/api/v1/geocode/reverse?lat=${lat}&lon=${lon}`);
        if (!res.ok) throw new Error(`Reverse geocode failed: ${res.status}`);
        const data = await res.json();
        // Nominatim display_name is very long - extract a shorter, readable address
        const fullAddress = data?.address || '';
        if (!fullAddress) return '';
        // Take only the first 2-3 meaningful parts (before the 4th comma)
        const parts = fullAddress.split(',').map(p => p.trim()).filter(Boolean);
        if (parts.length <= 3) return fullAddress;
        // Return: street/building, ward/district, city
    ...
    const ensureLocation = async () => {
        ...
        try {
            ...
            const finalLoc = address ? address : `Tọa độ: ${coordStr}`;
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (!storedUser) {
                ...
                if (search.includes('sessionId') && search.includes('token')) {
                    navigate(`/login?redirect=${encodeURIComponent('/student/scanner' + search)}`);
                } else {
                    alert('Vui lòng đăng nhập để sử dụng tính năng quét mã QR!');
                    navigate('/login');
                }
                return;
            }
            ...
            if (s.role === 'TEACHER') {
                alert('Tính năng này chỉ dành cho sinh viên!');
                navigate('/teacher/timetable');
                return;
            }
            if (s.role !== 'STUDENT') {
                alert('Tài khoản không có quyền truy cập tính năng này!');
                navigate('/login');
                return;
            }
